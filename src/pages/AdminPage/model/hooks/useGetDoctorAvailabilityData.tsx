...
export const useGetDoctorAvailabilityData = () => {
	...
	const fetchDoctorAvailabilityData = async () => {
		...
		for (const clinic of clinics?.searchClinic.elems || []) {
			const { data } = await searchClinicDoctors({ variables: { clinicId: clinic.id, searchStr: "" } });
			doctors.push(...((data?.searchClinicDoctor.elems as Doctor[]) || []));
		}

		const avaiblability: DoctorAvailability[] = [];
		...
		for (const doctor of doctors) {
			...
			avaiblability.push(...((data?.searchClinicDoctorAvailability.elems as DoctorAvailability[]) || []));
			return avaiblability;
		}
	};

	return { isLoading: ClinicLoading || DoctorLoading || AvaibleLoading, fetchDoctorAvailabilityData };
};
