...
import { format } from "date-fns"; // Убедитесь, что date-fns установлен
...
import {
    ...
    SearchClinicTableQueryVariables,
    UpdateClinicTableDocument,
} from "@/shared/__generate/graphql-frontend";
import { Link } from "react-router-dom";
import { getRouteViewClientPage } from "@/shared/consts/router";
...
const DoctorPage: React.FC = () => {
    ...
    const changeInputParameters = (params: Partial<{ startDate: string; endDate: string; comment: string; showArchive: boolean }>) => {
    ...
    return (
        <div style={{ padding: "20px" }}>
                        ...
                        <List.Item key={appointment.id}>
                            <Card style={{ backgroundColor: "#282c34", color: "white", marginBottom: "10px" }}>
                                <Text style={{ color: "white" }}>
                                    {format(new Date(appointment.beginDate), "dd MMM yyyy, HH:mm", { locale: ru })} -{" "}
                                    {format(new Date(appointment.endDate), "dd MMM yyyy, HH:mm", { locale: ru })}
                                </Text>
                                ...
                                <p style={{ color: "white" }}>Доктор: {doctorName}</p>
                                {inputParameters.showArchive && appointment.comment && (
                                    <p style={{ color: "white" }}>Сводка: {appointment.comment}</p>
                                )}
            ...
            >
                <TextArea rows={4} value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Сводка" />
            </Modal>
        </div>
    );
};
